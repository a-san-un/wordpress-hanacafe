...
| `menu-sitemap.xml` | — なし | CPT menuはYoastで**意図的に**noindex設定済みのため生成されない（正常） |

> ❗ `/news/` や `/menu/` の固定ページが `page-sitemap.xml` に入っていないのは、Yoastの認識がないため。実容上の問題はない。
...
| # | 内容 | 状態 |
| --- | --- | --- |
| 12-1 | `04_roadmap.md` を新規作成・旧04をarchiveへ移動 | ✅ |
...
| 12-6 | `10_deploy-checklist.md §1〜§6` を `10_公開前チェックリスット.md` として独立・旧10をarchiveへ | ✅ |
...
| 13-2 | Google Search Console — カバレッジエラー0件を確認 | ✅（2026-04-03確認・エラー0件・ concept/はクロール待ち（正常）） |
...
| STEP 13 公開後モニタリング | 7件 | **全件完了 ✅（2026-04-03）** |

> 📝 次の改善・保守タスクが発生した際は、STEP 14 以降に追加する。
