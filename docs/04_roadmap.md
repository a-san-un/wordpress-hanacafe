...
目的: ローカル環境での最終確認を経て、本番サーバーへ移行する。
チェックリスト詳細: [docs/10_deploy-checklist.md](./10_deploy-checklist.md)

| #     | 対象 | 内容 | 状態 |
| ----- | ---- | ---- | ---- |
| 11-1  | `screenshot.png` | テーマフォルダーに配置 | ✅ |
| 11-2  | `docs/10_deploy-checklist.md` | セクション7（本番移行）更新 | ✅ |
| 11-3  | WordPress 管理画面 | ACFマスターデータ入力（17項目） | ✅ |
| 11-3+ | `functions.php` / `robots.txt` | データ保管専用5ページに noindex 設定・robots.txt 配置 | ✅ |
| 11-4  | ブラウザ実機 | ローカル表示・動作確認（セクション3 全13項目） | 🔲 |
| 11-5  | ブラウザ実機 | セキュリティ確認（セクション4） | 🔲 |
| 11-6  | ブラウザ実機 | SEO・アクセシビリティ確認（セクション5） | 🔲 |
| 11-7  | ブラウザ実機 | パフォーマンス確認（セクション6） | 🔲 |
| 11-8〜11-11 | 本番サーバー | 本番移行（All-in-One WP Migration）（セクション7） | 🔲 |
...
| コミット | 内容 |
| -------- | ---- |
| 651a5ff  | docs: 固定ページ一覧を追加、robots.txtを配置 |
