...
* [1. プロジェクト概要](<HanaCAFE nappa69 プロジェクト仕様書.md#1-プロジェクト概要>)
* [2. テンプレート階層図](<HanaCAFE nappa69 プロジェクト仕様書.md#2-テンプレート階層図>)
* [3. データ構造図（CPT / ACF 関連図）](<HanaCAFE nappa69 プロジェクト仕様書.md#3-データ構造図cpt--acf-関連図>)
* [4. ACF フィールド完全一覧](<HanaCAFE nappa69 プロジェクト仕様書.md#4-acf-フィールド完全一覧>)
  * [A. common-info（page\_id=218）](<HanaCAFE nappa69 プロジェクト仕様書.md#a-common-infopage_id218>)
  * [B. access-info（page\_id=146）](<HanaCAFE nappa69 プロジェクト仕様書.md#b-access-infopage_id146>)
  * [C. menu-info（page\_id=213）](<HanaCAFE nappa69 プロジェクト仕様書.md#c-menu-infopage_id213>)
  * [D. about-seats（page\_id=184）](<HanaCAFE nappa69 プロジェクト仕様書.md#d-about-seatspage_id184>)
  * [E. CPT menu 投稿フィールド](<HanaCAFE nappa69 プロジェクト仕様書.md#e-cpt-menu-投稿フィールド>)
* [5. FLOCSS 設計ルール](<HanaCAFE nappa69 プロジェクト仕様書.md#5-flocss-設計ルール>)
  * [ディレクトリ構成](<HanaCAFE nappa69 プロジェクト仕様書.md#ディレクトリ構成>)
  * [Foundation（基盤）](<HanaCAFE nappa69 プロジェクト仕様書.md#foundation基盤>)
  * [Layout（骨格）](<HanaCAFE nappa69 プロジェクト仕様書.md#layout骨格>)
  * [Object / Component（汎用部品）](<HanaCAFE nappa69 プロジェクト仕様書.md#object--component汎用部品>)
  * [Object / Project（ページ固有）](<HanaCAFE nappa69 プロジェクト仕様書.md#object--projectページ固有>)
* [6. 主要ロジック仕様（functions.php）](<HanaCAFE nappa69 プロジェクト仕様書.md#6-主要ロジック仕様functionsphp>)
* [7. JavaScript 仕様（main.js）](<HanaCAFE nappa69 プロジェクト仕様書.md#7-javascript-仕様mainjs>)
* [8. 関連ドキュメント一覧](<HanaCAFE nappa69 プロジェクト仕様書.md#8-関連ドキュメント一覧>)
...
| フォント        | Montserrat + Noto Sans JP（本文）/ Montserrat + Noto Serif JP（見出し） |
...
### C. menu-info（page\_id=213）

グループID: `group_69ae83eea8ead`

| フィールド名            | スラッグ               | 型            | 役割・出力先テンプレート                                                                                                                                |
| ----------------- | ------------------ | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------- |
| TOPピックアップ MEAL    | `top_menu_meal`    | post\_object | `template-parts/home/menu.php` の MEAL カード1件。`get_hanacafe_top_menu_post()` 経由でACF未設定時も `WP_Query`（taxonomy: `menu_category:meal`）でフォールバック取得 |
| TOPピックアップ DRINK   | `top_menu_drink`   | post\_object | 同上（taxonomy: `menu_category:drink`）                                                                                                         |
| TOPピックアップ DESSERT | `top_menu_dessert` | post\_object | 同上（taxonomy: `menu_category:dessert`）                                                                                                       |

***

### D. about-seats（page\_id=184）
...
| テラス席 ペット可   | `is_pet_terrace`      | true\_false | `true` 時に `Pet Friendly` バッジ（`.c-badge-feature`）を表示。terrace にのみ定義 |
...
| おすすめフラグ   | `is_recommended` | true\_false | `.c-badge--recommend` の表示ON/OFF。`WP_Query` の `meta_query` `orderby` に使用               |
...
src/scss/
├── global/
│   ├── _variables.scss   ← デザイントークン（色・フォント・z-index・余白）の唯一の定義元
...
| z-index     | `$z-header: 100`, `$z-drawer: $z-header + 1`              | 全レイヤーをここで一元管理                 |
...
| ドキュメント                | パス                                                                          | 主な内容                                                         |
| --------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------ |
| **CMS運用マニュアル**        | [`docs/09_cms運用マニュアル.md`](09_cms運用マニュアル.md)                                 | WordPress管理画面での日常運用手順（メニュー追加・座席状態更新等）の操作ガイド                  |
| **本番移行手順書**           | [`docs/08_本番移行手順書.md`](08_本番移行手順書.md)                                       | All-in-One WP Migration による本番デプロイ手順・DNS設定・wp-config.php 変更手順 |
| **アーキテクチャ・設計ルール**     | [`docs/01_architecture-and-design-rules.md`](./)                            | テーマ全体の設計思想・FLOCSS / BEM ルール・CSS SSOT方針                       |
| **WordPressコーディング規約** | [`docs/02_wordpress-coding-standards.md`](02_wordpress-coding-standards.md) | PHP・SCSS・JS のコーディング規約・エスケープ関数使用ルール・Git運用ルール                  |
| **ロードマップ**            | `docs/04_roadmap.md`                                                        | 開発フェーズと今後の機能追加計画                                             |
| **SEO動作確認チェックリスト**    | [`docs/10_SEOチェックリスト.md`](10_サイト品質チェックリスト.md)                               | Chrome DevTools を使った本番SEO確認スクリプト集                            |
...
_本仕様書は `acf-fields.json` / `cptui-post-types.json` / `functions.php` / `src/scss/` を一次情報源とする。コードの実態と乖離を発見した場合は、コードを優先し本仕様書を更新すること。_
